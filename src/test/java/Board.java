...
public class Board {
	...
	public Board(){
		...
		board[7] = new Tile("Ground thaws. Start plowing early. Go directly to Spring planting.", (player) -> player.moveTo(14,false));
		board[8] = new Tile("Rainy day. Draw O.T.B.", drawOTB);
		board[9] = new Tile("Becomes obvious your wheat has winter killed. PAY $2000 to replant.", -2000);
		board[10] = new Tile("Start plowing late. PAY $500", -500);
		board[11] = new Tile("Hurt your back. Go back to the second week of January.", (player) -> player.moveTo(2,false));
		board[12] = new Tile("Frost orces you to heat fruit. PAY $2000 if you own fruit.", (player) -> {if (player.acreage(Crop.fruit)>0) player.subMoney(2000);});
		...
		board[21] = new Tile(Crop.hay, "1st Hay Cutting", "Rain storm ruins unbaled hay. Cut your harvest check in half.", .5);
		board[22] = new Tile(Crop.hay, "1st Hay Cutting", "Good growing weather. COLLECT $500 bonus", 500);
		
		board[23] = new Tile(Crop.fruit, "Cherry Harvest", "Rain splits your cherries. Cut your harvest check in half.",.5);
		board[24] = new Tile(Crop.fruit, "Cherry Harvest", "Dust storm. Draw Farmer's fate", drawFF);
		board[25] = new Tile(Crop.fruit, "Cherry Harvest", "INDEPENDENCE DAY BASH",null);
		...
		board[28] = new Tile(Crop.hay, "2nd Hay Cutting", "It's a cooker! 114 in the shade. Wipe your brow and go to Harvest Moon after getting Hay check.", (player) -> player.moveTo(36, true));
		
		board[29] = new Tile(Crop.grain, "Wheat Harvest", "85, wheat heads filling out beautifully. Add $50 per acre to your harvest check.", (player) -> player.addMoney(player.acreage(Crop.grain)*50));
		...
		board[34] = new Tile(Crop.hay, "3rd Hay Cutting", "Tractor owners: bale Hay, then go to the third week of November. Collect your $1000 there, then harvest your Fruit.", (player) -> player.moveTo(45, true));
		...
		board[37] = new Tile(Crop.livestock, "Livestock Sales","Market collapses. Cut livestock check in half.",.5d);
		board[38] = new Tile(Crop.livestock, "Livestock Sales","Codling Moth damage to apples lowers fruit grade. Pay $2000 if you own fruit.", (player) -> {if (player.acreage(Crop.fruit)>0) player.subMoney(2000);});
		...
		board[46] = new Tile(Crop.grain, "Corn Harvest","Early freeze kills fruit buds. PAY $1000 if you have fruit.", (player) -> {if (player.acreage(Crop.fruit)>0)player.subMoney(1000);});
